...
def publish_article_to_web(item, article_text, attachment_name=""):
    title = (item.get("title") or "Public Health Update").strip()
    ...
    with sqlite3.connect(WEB_DB_PATH) as web_conn:
        ...
        if existing:
            ...
            if len(content.split()) > len(existing_content.split()):
                web_conn.execute(
                    ...
                    SET content = ?, url = ?, attachment_name = ?, created_at = ?
                    WHERE id = ?
                    """,
                    (content, url, attachment_name, created_at, existing[0]),
                )
                web_conn.commit()
                return True
            return False
        ...
        web_conn.execute(
            ...
            (title, source, article_date, url, content, status, attachment_name, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (title, source, article_date, url, content, "pending", attachment_name, created_at),
        )
        web_conn.commit()
